function N2O_Tank = Ox_Tank_Update(N2O_Tank, Comb_Chamber, N2O_Valve, dt)
...
mdot_tank_outflow = N2O_Tank(11);
    ...
    if (tank_liquid_mass < .0001) || (first_vapour_it == 0)
        %if tank has almost no liquid left, use the vapour routines
         N2O_Tank = tank_no_liquid(N2O_Tank, Comb_Chamber, N2O_Valve, dt);
    else
        %otherwise, use the liquid/vaopur mix routines
        N2O_Tank = tank_with_liquid(N2O_Tank, Comb_Chamber, N2O_Valve, dt);
    end
end